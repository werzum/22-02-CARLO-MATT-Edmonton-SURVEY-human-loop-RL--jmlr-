...
First, we argue that RL is a human-in-the-loop paradigm. There are three distinct stages of where human involvement is critical.
...
Second, in each of these three stages, we argue that explainability is a critical (underdeveloped) technology. Before training, explainability can help show the impact of algorithm or hyperparameter selection, how prior knowledge biases the agent, and how pre-training changes the agent's learning process. During training, explainability can show the impact biasing via an existing controller or human advice, how learning is progressing, or how the current policy functions. After training, explainability can help explain the final policy, improve trust, help the person evaluate safety, and understand the policy's stability.
...
%After the introduction and motivation, we provide in section \ref{sec:background} a thorough overview on background work for Explainability, Interactive and Causal Learning, and review fundamental and current challenges for embodied intelligence. Then, we explore general considerations for enabling the Human-Robot teamup, followed by the application phases Development, Teaching, Testing and Using, each with specific requirements for the teamup to succeed. Finally, in the conclusion, we outline current problems and goals in the field, and propose further future work.
...
%Safety - Regulations / Norms (Alessa)
...
%%% Justify Explainability and Relation with RL
...
% Inform user and allow him to also give knowledge into the system
...
%Overview
    % 1. Topics: Explainable learning process, requires techniques that work already on emerging models
    % 2. Focus: Interpretable models (ensure sensible groundwork before extensive training)
    % 3. xAI approaches: causal and textual explanations (which are rapid to compute and work without extensive training) - interpretable approaches could here provide inherent understandability
...
%something along those lines: https://www.capitalone.com/tech/machine-learning/10-common-machine-learning-mistakes/
...
Especially approaches that allow to summarise a general model policy seem fitting for the development phase.
One approach would be to codify decision process as rules, as seen in the linear model u-trees by \citet{LiuEtAl:2018:LinearModelUTrees}. These networks aim to make the feature influence and rules learned by the network more transparent.
Another approach is to represent the learned DRL model with generated code blocks, as presented by \citet{VermaEtAl:2018:ProgrammaticallyInterpretableRL}. Their `ProgrammaticallyInterpretable RL" first learns a neural policy network and then searches a programmatic policy that adequately codifies this network. This approach incurs a performance hit during training, but results in human-readable policies and improves generalization. 
A third approach is to represent the network policies via natural language. \citet{AlonsoEtAl:2018:xAINLBeerClassifier} show an example of justifying classifications with a textual explanation of the choice made by a decision tree. 
...
% Overview
    ...
    % 1. Topics: Input evaluation approaches, frameworks to enable and facilitate interaction (HIPPOGym, Pebble)
    % 2. Focus: Interpretable inputs (what does the agent perceive)
    % 3. xAI: Visualization of inputs (activation images), counterfactuals for rapid policy evaluation, symbolic representations
...
\section{Using RL Agents}

% Here, benefits from previous steps and general benefits from teamup can be seen
...
% Developing - model code summaries
...
% Teaching - many options, PBL, CFs, Imitation Learning
...
% Testing - Policy Summarization and Active Learning
...
% Using - Low-tech action indication, showing model perception
...
%think about how to adapt!

% Developing - using compositional and representational languages, causal learning
...
% Teaching - adapt approaches to RL contexts
...
% Testing - ensure approaches are scalable and expert-readable
...
% Using - modify for other contexts than autonomous driving, adapt to other modalities than dashboard
