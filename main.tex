...
% Motivate RL a little better - Payam
...
% Integrate Policy summarization and Policy querying into 3 categories below -Payam:
...
% Current Structure
    % What is IL
    % Four dimension of IL; reward shaping
    % Policy shaping and the other dimensions
    % Action Pruning/////
    % UI
...
We argue that these challenges show that generalisable and performant RL is a fundamentally challenging problem, as exemplified by the different challenges detailed in this section. Challenges like slow learning speed hinder its applicability \citep{Li:2019:HumanCenteredRLSurvey}. We claim that many of those challenges can be overcome with the application of HITL approaches. Similarly, \citep{MatthewsonPilarski:2022:DesigningAndEvaluatingHCIML} argue that human-centered thinking for RL approaches is essential for designing and deployment of RL systems, enabled by human-centered approaches like reward shaping and better feedback between agent and human \citep{Li:2019:HumanCenteredRLSurvey}.
...
%RL needs HITL
In the following, we summarize the findings and outcomes of our survey. We emphasize the idea that Reinforcement Learning is a fundamentally difficult problem, which greatly benefits from the advantages of HITL approaches.
%example for HITL
%HITL needs xAI
We furthermorge argue that HITL approaches benefit greatly from xAI approaches, since all of these are fundamentally human approaches.
%argue for xAI requirement.
%four stages+requirements
We identified we stages for the deployment of HITL RL solutions and discussed how xAI can be beneficial in these situations and what should be considered for a successful deployment.
% Brief stages
% HITL combination increases productivity and enables human robot teamup
We ultimately argue that the combination of RL and HITL with xAI approaches should be a major focus for increasing productivity in real-life applications of RL, and discuss different blue-sky approaches for current xAI methods. We furthermore propose the vision of an interactive human-robot teamup, which enables new use-cases for RL applications and allows both humans and robots to leverage their full potential and respective strengths.
